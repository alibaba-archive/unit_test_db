...
/**
 * 实现和XStream一摸一样，只是修改了一些属性和方法的可见性，便于ExXstream继承重载
 * 
 * @author darui.wudr
 */
@SuppressWarnings({ "unchecked", "deprecation" })
public class AccessibleXStream {
    ...
    protected ClassLoaderReference        classLoaderReference;
    ...
    protected transient JVM               jvm                       = new JVM();
    ...
    /**
     * Constructs a default XStream. The instance will use the {@link XppDriver}
     * as default and tries to determine the best match for the
     * {@link ReflectionProvider} on its own.
     * 
     * @throws InitializationException in case of an initialization problem
     */
    public AccessibleXStream() {
        this(null, (Mapper) null, new XppDriver());
    }

    /**
     * Constructs an XStream with a special {@link ReflectionProvider}. The
     * instance will use the {@link XppDriver} as default.
     * 
     * @throws InitializationException in case of an initialization problem
     */
    public AccessibleXStream(ReflectionProvider reflectionProvider) {
     ...
     * @throws InitializationException in case of an initialization problem
     */
    public AccessibleXStream(HierarchicalStreamDriver hierarchicalStreamDriver) {
     ...
     * @throws InitializationException in case of an initialization problem
     */
    public AccessibleXStream(ReflectionProvider reflectionProvider, HierarchicalStreamDriver hierarchicalStreamDriver) {
     ...
     */
    public AccessibleXStream(ReflectionProvider reflectionProvider, ClassMapper classMapper,
     ...
     * @deprecated As of 1.2, use
     *             {@link #XStream(ReflectionProvider, Mapper, HierarchicalStreamDriver)}
     *             and register classAttributeIdentifier as alias
     */
    public AccessibleXStream(ReflectionProvider reflectionProvider, ClassMapper classMapper,
    ...
    /**
     * Constructs an XStream with a special {@link HierarchicalStreamDriver} and
     * {@link ReflectionProvider} and additionally with a prepared
     * {@link Mapper}.
     * 
     * @throws InitializationException in case of an initialization problem
     * @deprecated since 1.3, use
     *             {@link #XStream(ReflectionProvider, HierarchicalStreamDriver, Mapper, ClassLoader)}
     *             instead
     */
    public AccessibleXStream(ReflectionProvider reflectionProvider, Mapper mapper, HierarchicalStreamDriver driver) {
     ...
     * @since 1.3
     */
    public AccessibleXStream(ReflectionProvider reflectionProvider, HierarchicalStreamDriver driver,
    ...
    private Mapper buildMapper() {
        ...
        if (JVM.is15()) {
            mapper = buildMapperDynamically(ANNOTATION_MAPPER_TYPE,
                    new Class[] { Mapper.class, ConverterRegistry.class, ClassLoader.class, ReflectionProvider.class,
                            JVM.class },
    ...
    protected void setupAliases() {
        ...
        if (jvm.supportsAWT()) {
            ...
            // undesirable.
            // Calling loadClass ensures a reference to the class is found but
            // they are not
            // instantiated.
    ...
    protected void dynamicallyRegisterConverter(String className, int priority, Class[] constructorParamTypes,
                                                Object[] constructorParamValues) {
        try {
            Class type = Class.forName(className, false, classLoaderReference.getReference());
            Constructor constructor = type.getConstructor(constructorParamTypes);
     ...
     * Serialize and object to a hierarchical data structure (such as XML).
     * 
     * @param dataHolder Extra data you can use to pass to your converters. Use
     *            this as you want. If not present, XStream shall create one
     *            lazily as needed.
     * @throws XStreamException if the object cannot be serialized
     */
     ...
     * a special use case! With the ReflectionConverter XStream will write
     ...
     * is a special use case! With the ReflectionConverter XStream will write
     * directly into the raw memory area of the existing object. Use with care!
     * 
     * @throws XStreamException if the object cannot be deserialized
     */
     ...
     * ReflectionConverter XStream will write directly into the raw memory area
     * of the existing object. Use with care!
     * 
     * @throws XStreamException if the object cannot be deserialized
     */
     ...
     * Deserialize an object from a hierarchical data structure (such as XML).
     * 
     * @param root If present, the passed in object will have its fields
     *            populated, as opposed to XStream creating a new instance.
     *            Note, that this is a special use case! With the
     *            ReflectionConverter XStream will write directly into the raw
     ...
     * @throws InitializationException if no {@link ClassAliasingMapper} is
     *             available
     */
     ...
     *            other specified.
     * @throws InitializationException if no
     *             {@link DefaultImplementationsMapper} or no
     ...
     * @throws InitializationException if no {@link FieldAliasingMapper} is
     *             available
     */
     ...
     * @throws InitializationException if no {@link AttributeAliasingMapper} is
     *             available
     */
    ...
    /**
     * Create an alias for a system attribute. XStream will not write a system
     ...
     * @throws InitializationException if no
     *             {@link SystemAttributeAliasingMapper} is available
     * @since 1.3.1
     */
     ...
     * @throws InitializationException if no {@link AttributeMapper} is
     *             available
     * @since 1.2
     */
     ...
     * @param ofType
     * @throws InitializationException if no
     *             {@link DefaultImplementationsMapper} is available
     */
     ...
     * 
     * @throws InitializationException if no {@link ImmutableTypesMapper} is
     *             available
     */
    ...
    /**
     * @throws ClassCastException if mapper is not really a deprecated
     *             {@link ClassMapper} instance
     ...
     * .
     * 
     * @throws IllegalArgumentException if the mode is not one of the declared
     *             types
        ...
        switch (mode) {
            ...
            case XPATH_RELATIVE_REFERENCES:
                setMarshallingStrategy(
                        new ReferenceByXPathMarshallingStrategy(ReferenceByXPathMarshallingStrategy.RELATIVE));
                break;
     ...
     * @throws InitializationException if no {@link ImplicitCollectionMapper} is
     *             available
     */
     ...
     * {@link #createObjectOutputStream(Writer, String)}.
     * </p>
     * 
     * @see #createObjectOutputStream(HierarchicalStreamWriter, String)
     * @see #createObjectInputStream(HierarchicalStreamReader)
     * @since 1.0.3
     */
     ...
     * @param rootNodeName The name of the root node enclosing the stream of
     *            objects.
     * @see #createObjectInputStream(HierarchicalStreamReader)
     * @since 1.0.3
     */
    ...
    /**
     * Change the ClassLoader XStream uses to load classes. Creating an XStream
     * instance it will register for all kind of classes and types of the
     * current JDK, but not for any 3rd party type. To ensure that all other
     * types are loaded with your classloader, you should call this method as
     * early as possible - or consider to provide the classloader directly in
     * the constructor.
     * 
     * @since 1.1.1
     */
