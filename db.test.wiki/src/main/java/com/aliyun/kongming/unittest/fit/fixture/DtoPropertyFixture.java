...
import com.aliyun.kongming.unittest.fit.exception.JTesterReflectionException;
...
import com.aliyun.kongming.unittest.fit.util.reflect.MethodAccessor;

import fit.Parse;
import fit.TypeAdapter;
import fit.exception.FitFailureException;

public class DtoPropertyFixture extends JTesterFixture {
    ...
    protected void findMethod(Parse cell) {
        ...
        try {
            if (argument == null) {
                this.accessor = MethodAccessor.findMethodByArgCount(this, methodname, 1);
    ...
    public void doRow(Parse row) {
        ...
        try {
            ...
            Object result = accessor.invoke(para);
    ...
    private Object instancePara(Parse cells) throws Exception {
        if (this.parsePropertyMethodName != null) {
            ...
            try {
                MethodAccessor<?> methodAccessor = new MethodAccessor<Object>(this, parsePropertyMethodName,
                        String.class);
                return methodAccessor.invoke(text);
            } catch (JTesterReflectionException e) {
    ...
    protected class PropertyBinding {
        ...
        public PropertyBinding(String property) throws Exception {
            this.fieldAccessor = new FieldAccessor(camel(property), dtoClazz);
