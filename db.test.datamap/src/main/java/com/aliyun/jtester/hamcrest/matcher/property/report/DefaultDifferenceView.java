package com.aliyun.jtester.hamcrest.matcher.property.report;

import static com.aliyun.ext.jtester.apache.commons.lang.ClassUtils.getShortClassName;
import static com.aliyun.jtester.hamcrest.matcher.property.report.DefaultDifferenceReport.MAX_LINE_SIZE;
import static com.aliyun.jtester.hamcrest.matcher.property.report.DefaultDifferenceReport.MatchType.NO_MATCH;

import java.util.List;
import java.util.Map;

import com.aliyun.jtester.hamcrest.matcher.property.difference.ClassDifference;
import com.aliyun.jtester.hamcrest.matcher.property.difference.CollectionDifference;
import com.aliyun.jtester.hamcrest.matcher.property.difference.Difference;
import com.aliyun.jtester.hamcrest.matcher.property.difference.DifferenceVisitor;
...
import com.aliyun.jtester.hamcrest.matcher.property.difference.UnorderedCollectionDifference;
import com.aliyun.jtester.hamcrest.matcher.property.difference.ClassDifference;
import com.aliyun.jtester.hamcrest.matcher.property.difference.CollectionDifference;
import com.aliyun.jtester.hamcrest.matcher.property.difference.Difference;
import com.aliyun.jtester.hamcrest.matcher.property.difference.DifferenceVisitor;
...
public class DefaultDifferenceView implements DifferenceView {
	...
	protected String formatDifference(CollectionDifference collectionDifference, String fieldName) {
		...
		for (Integer leftIndex : collectionDifference.getLeftMissingIndexes()) {
			...
			result.append(formatValues(innerFieldName, leftList.get(leftIndex), DefaultDifferenceReport.MatchType.NO_MATCH));
		}
		for (Integer rightIndex : collectionDifference.getRightMissingIndexes()) {
			...
			result.append(formatValues(innerFieldName, DefaultDifferenceReport.MatchType.NO_MATCH, rightList.get(rightIndex)));
		}
		return result.toString();
	}
	...
	protected String formatDifference(UnorderedCollectionDifference unorderedCollectionDifference, String fieldName) {
		...
		for (Map.Entry<Integer, Integer> bestMatchingIndex : bestMatchingIndexes.entrySet()) {
			...
			if (leftIndex == -1) {
				...
				result.append(formatValues(innerFieldName, DefaultDifferenceReport.MatchType.NO_MATCH,
			...
			if (rightIndex == -1) {
				...
				result.append(formatValues(innerFieldName, unorderedCollectionDifference.getLeftList().get(leftIndex),
						DefaultDifferenceReport.MatchType.NO_MATCH));
				continue;
			}
	...
	protected String formatValues(String fieldName, Object leftValue, Object rightValue) {
		...
		if (valuesFormattedOnOneLine.length() < DefaultDifferenceReport.MAX_LINE_SIZE) {
