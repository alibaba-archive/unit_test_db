...
/**
 * Comparator that can handle Hibernate proxies.<br>
...
public class HibernateProxyComparator implements Comparator {
    ...
    /**
     * Compares the given objects. If one of the objects is a proxy, the proxy
     * is initialized and the wrapped values are compared. If both objects are
     * proxies and both objects are not yet loaded (initialized) only the
     * idendtifiers are compared. This avoids performing unnecessary loads from
     * the database (potentially retrieving a huge amount of data). If the ids
     * of the proxies are identical, the objects are considered identical, if
     * not the objects are considered different.
     *
        ...
        if (HibernateUtil.isUninitialized(left) && HibernateUtil.isUninitialized(right)) {
            ...
            if (leftType == null || !leftType.equals(rightType)) {
                return new ObjectDifference(
                        "Different hibernate proxy types. Left: " + leftType + ", right: " + rightType, left, right);
            }

            Object leftIndentifier = HibernateUtil.getIdentifier(left);
            Object rightIdentifier = HibernateUtil.getIdentifier(right);
            Difference identifierDifference = reflectionComparator.getDifference(leftIndentifier, rightIdentifier,
                    onlyFirstDifference);
